...
function showPhoneNums(phoneNumbers){
	...
	for (let [k,v] of phoneNumbers.entries()){
		console.log(k + ': ' + v);
	}
...
function showTransactions(accountTransactions, balance){
	...
	for (let [k, v] of accountTransactions){
		balance = balance + v;
		if (balance < 0){
			balance = balance + 25;
		} else if (v < 0){
			console.log(`${k} you withdraw ${v} your new balance is ${balance}`);
		} else {
			console.log(`${k} you deposited ${v} your new balance is ${balance}`);
		}
	}
	return showBalanceStatus(balance)
}
